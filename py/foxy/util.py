...
def is_in_bounds(pos : tuple[int, int]) -> bool:
	return 0 < pos[0] < WIN_WIDTH and 0 < pos[1] < WIN_HEIGHT

def play_gif(
		base_name : str, dest : tuple[int, int, int, int],
		time_interval : float,
		begin : int, end : int
		) -> None:
	"""
	Blit the images "{base_name}{from}.png", "{base_name}{from + 1}.png", ..., "{base_name}{to}.png" onto window for time_interval seconds each.
